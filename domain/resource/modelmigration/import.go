...
func (i *importOperation) Execute(ctx context.Context, model description.Model) error {
	var args []resource.SetResourcesArg
	apps := model.Applications()
	for _, app := range apps {
		resources := app.Resources()
		appArgs := resource.SetResourcesArg{
		...
		for _, unit := range app.Units() {
			...
			for _, res := range unitResources {
				...
				appArgs.UnitResources = append(appArgs.UnitResources, resource.SetUnitResourceInfo{
	...
	err := i.resourceService.SetResources(ctx, args)
	if err != nil {
...
func importResourceRevision(name string, rev description.ResourceRevision) (resource.SetResourceInfo, error) {
	if name == "" {
		return resource.SetResourceInfo{}, errors.Errorf("got empty resource name: %w", resourceerrors.ResourceNameNotValid)
	}
	origin, err := charmresource.ParseOrigin(rev.Origin())
	if err != nil {
		return resource.SetResourceInfo{}, errors.Errorf("parsing origin: %w: %w", resourceerrors.OriginNotValid, err)
	}
	revision := rev.Revision()
	switch origin {
	case charmresource.OriginStore:
		if revision < 0 {
			return resource.SetResourceInfo{}, errors.Errorf(
	...
	return resource.SetResourceInfo{
		Name:      name,
		Origin:    origin,
		Revision:  rev.Revision(),
