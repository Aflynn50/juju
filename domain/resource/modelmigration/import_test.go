...
func (s *importSuite) TestEmptyImport(c *gc.C) {
	...
	s.resourceService.EXPECT().SetResources(gomock.Any(), nil)
...
func (s *importSuite) TestImport(c *gc.C) {
	...
	s.resourceService.EXPECT().SetResources(gomock.Any(), []domainresource.SetResourcesArg{{
		ApplicationName: appName,
		Resources: []domainresource.SetResourceInfo{{
			Name:      res1Name,
			Origin:    res1Origin,
			Revision:  res1Revision,
			Timestamp: res1Time,
		...
		UnitResources: []domainresource.SetUnitResourceInfo{{
