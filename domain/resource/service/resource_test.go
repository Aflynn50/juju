...
func (s *resourceServiceSuite) TestStoreResource(c *gc.C) {
	...
	s.state.EXPECT().StoreResource(gomock.Any(), resourceUUID, storageID, retrievedBy, retrievedByType, false)
...
func (s *resourceServiceSuite) TestStoreResourceAndIncrementCharmModifiedVersion(c *gc.C) {
	...
	s.state.EXPECT().StoreResource(gomock.Any(), resourceUUID, storageID, retrievedBy, retrievedByType, true)
...
func (s *resourceServiceSuite) TestOpenResourceBadSize(c *gc.C) {
	defer s.setupMocks(c).Finish()
	id := resourcetesting.GenResourceUUID(c)
	reader := io.NopCloser(bytes.NewBufferString("spam"))
	resourceType := charmresource.TypeFile
	...
	s.resourceStore.EXPECT().Get(
		gomock.Any(),
		id.String(),
	).Return(reader, badSize, nil)

	_, _, err = s.service.OpenResource(context.Background(), id)
	c.Assert(err, gc.ErrorMatches, "retrieved resource size does not match expected resource size.*")
}
