	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) (err error) {
		...
		for _, res := range resources {
			...
			if hasBeenPolled {
				...
				if err != nil {
					return errors.Errorf("converting to charm resource: %w", err)
				}
			}
...
func (st *State) StoreResource(
	...
	storageUUID coreresourcestore.UUID,
	retrievedBy string,
	...
	resourceID := resourceIdentity{UUID: resourceUUID}
	kind := resourceKind{}
...
JOIN charm_resource_kind AS crk ON r.charm_uuid = cr.charm_uuid AND r.charm_resource_name = cr.name
WHERE r.uuid = $resourceIdentity.resource_uuid
`, resourceID, kind)
	if err != nil {
		return errors.Capture(err)
	}
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		err := tx.Query(ctx, getResourceType, resourceID).Get(&kind)
		if errors.Is(err, sqlair.ErrNoRows) {
			return errors.Errorf("resource %s: %w", resourceerrors.ResourceNotFound)
		} else if err != nil {
			return errors.Capture(err)
		}

		parsedKind, err := charmresource.ParseType(kind.Name)
		if err != nil {
			return errors.Errorf("parsing resource kind: %w", err)
		}

		switch parsedKind {
		case charmresource.TypeFile:
			err = st.insertFileResource(ctx, tx, resourceUUID, storageUUID)
		case charmresource.TypeContainerImage:
			err = st.insertContainerImageMetadata(ctx, tx, resourceUUID, storageUUID)
		default:
			return errors.Errorf("unknown resource type: %q", parsedKind.String())
		}
		if err != nil {
			return errors.Capture(err)
		}
...
// updateRetrievedByIfEmpty checks if there is already an entry for a resource
// in the retrieved_by table, and if not, adds the given retrieved by name and
// type.
func (st *State) updateRetrievedByIfEmpty(
	ctx context.Context,
	tx *sqlair.TX,
	resourceUUID coreresource.UUID,
	retrievedBy string,
	...
	// Prepare statement to verify if the resource has already been retrieved.
...
WHERE resource_uuid = $resourceIdentity.uuid`
	...
	retrievedByParam := setRetrievedBy{ResourceUUID: resourceUUID.String(), Name: retrievedBy}
...
// SetApplicationResource marks existing resource as in use by a CAAS
// application.
...
SELECT &kubernetesApplicationResource.* FROM kubernetes_application_resource
...
`, k8sAppResource)
	...
	resourceID := resourceIdentity{UUID: resourceUUID.String()}
	checkValidResourceStmt, err := st.Prepare(`
SELECT &resourceIdentity.uuid
FROM resource
WHERE uuid = $resourceIdentity.uuid
`, resourceID)
...
`, k8sAppResource)
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		err = tx.Query(ctx, checkValidResourceStmt, resourceID).Get(&resourceID)
		if errors.Is(err, sqlair.ErrNoRows) {
			return errors.Errorf("resource %s: %w", resourceID.UUID, resourceerrors.ResourceNotFound)
		}
		if err != nil {
			return errors.Capture(err)
		}
...
// LinkStoredResource links a storageUUID to a resourceUUID. This is the one
// that sets the retrieved by right.
func (st *State) LinkStoredResource(
	ctx context.Context,
	resourceUUID coreresource.UUID,
	storageUUID coreresourcestore.UUID,
) error {
...
func (st *State) LinkStoredResourceAndMarkChanged(
	ctx context.Context,
	resourceUUID coreresource.UUID,
	storageUUID coreresourcestore.UUID,
...
FROM resource
WHERE uuid = $resourceIdentity.uuid`
	checkValidResourceStmt, err := st.Prepare(checkValidResourceQuery, resourceID)
	if err != nil {
		return errors.Capture(err)
	}
